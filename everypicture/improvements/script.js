(function() {
    ...
    img.addEventListener("mousemove", function(e) {
        ...
        // update state of focused
        focused = blurValue < 1;
    });

    // reset status of photo if the mouse is moved away
    img.addEventListener("mouseleave", function() {
        img.style.filter = "blur(14px)";
        focused = false;
    });

    // function that integrates the element change once the image is focused and clicked on
    img.addEventListener("click", function() {

        // stops function if the image isn't focused or the timer hasn't started
        if (!focused || startTimer) return;
            startTimer = true;
            ...
            setTimeout (function () {
                ...
                startTimer = false; 
                focused = false;
            }, 10); // pretty irrelevant timing, but I kept it since it was an example technique from the slides
    });
